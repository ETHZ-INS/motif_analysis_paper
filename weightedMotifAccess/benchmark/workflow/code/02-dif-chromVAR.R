...
run_once <- function(x, genome, motif, seed) {
    ...
    tmp <- differentialDeviations(dev, groups = group_id)
    mat <- assay(dev, "deviations")
    tvals <- apply(mat, 1, function(x) {
        t.test(x[group_id == "B"], x[group_id == "A"])$statistic
    })
    ...
    res$P.Value <- tmp$p_value
    res$adj.P.Val <- tmp$p_value_adjusted
    res$t <- tvals
